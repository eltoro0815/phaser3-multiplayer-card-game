...
export default class SocketHandler {
    constructor(scene) {
        ...
        // scene.socket = io('https://phaser3-tabletop-card-game-2021.herokuapp.com/');
            ...
            if (socketId === scene.socket.id) {
                for (let i in cards) {
                    let card = scene.GameHandler.playerHand.push(
                        scene.DeckHandler.dealCard(155 + (i * 155), 860, cards[i], "playerCard")
                    );
                }
            }
            else {
                for (let i in cards) {
                    let card = scene.GameHandler.opponentHand.push(
                        scene.DeckHandler.dealCard(155 + (i * 155), 135, "cardBack", "opponentCard")
                    );
                }
            }
        })
